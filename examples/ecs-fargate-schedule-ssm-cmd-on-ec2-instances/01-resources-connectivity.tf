...
resource "aws_security_group_rule" "app_sg_ec2_connect_in_ssh" {
  description       = "Allow inbound traffic from the IP range of AWS EC2_INSTANCE_CONNECT at ${var.aws_region}"
  security_group_id = aws_security_group.app_sg.id
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
  cidr_blocks       = ["${var.ec2conn_cidr_block}"]
}
